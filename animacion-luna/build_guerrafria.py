"""Render de LA GUERRA FRÍA, estilo pizarra (mismo estilo definitivo del alunizaje).
...
Se renderiza por tramos:
    START=0 END=180 OUT=/tmp/chunks/p00.mp4 python3 build_guerrafria.py
"""
import json
import os
import subprocess
...
W, H, FPS = 1280, 720, 30
AUDIO = '/mnt/documents/gf/full.wav'
MARKS = json.load(open('/mnt/documents/gf/marks_full.json'))
DUR = MARKS[-1]['t1'] + 2.0

START = float(os.environ.get('START', 0))
END = min(float(os.environ.get('END', DUR)), DUR)
OUT = os.environ.get('OUT', '/mnt/documents/guerra_fria_completo.mp4')
